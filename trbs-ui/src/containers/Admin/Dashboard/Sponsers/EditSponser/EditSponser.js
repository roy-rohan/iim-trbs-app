...
import * as classes from "./EditSponser.module.css";
import { initalValues } from "./InitialData";

class EditSponser extends Component {
  state = {
  ...
  componentDidMount() {
    ...
    axios
      .get("/sponsers/read-single.php?id=" + queryParams.sponserId)
      .then((response) => {
        let sponserData = response.data;
        let updatedValues = { ...this.state.initialValues };
        for (let attr in updatedValues) {
          updatedValues[attr].value = sponserData[attr];
        }
  ...
  onFormSubmit(sponser) {
    ...
    if (isValid) {
      ...
      if (this.state.initalValues) {
        ...
        this.props.onEditSponser(sponserData, this.uploadImages.bind(this));
      } else {
        this.submitBtnRef.current.textContent = "Submit";
      }
    } else {
  ...
  uploadImages() {
    if (this.state.sponserImage !== null) {
      ...
      sponserImageformData.append(
        "entity_id",
        this.state.initialValues.sponser_id.value
      );
      if (sponserImageformData.get("path") === "null") {
        uploadNewImage(sponserImageformData, () => {
          this.setState({
            ...this.state,
            sponserImageUploadSuccess: true,
          });
        });
      } else {
        updateImage(sponserImageformData, () =>
    ...
    if (this.state.sponserTimelineImage !== null) {
      ...
      timelineSponserImageformData.append(
        "entity_id",
        this.state.initialValues.sponser_id.value
      );
      if (timelineSponserImageformData.get("path") === "null") {
        // image was not present sponser when added, now being added first time
        ...
        updateImage(timelineSponserImageformData, () =>
          this.setState({
            ...this.state,
            sponserTimelineImageUploadSuccess: true,
          })
  ...
  successHandler() {
    this.props.onSendMessage("Sponser Edited Successfully", "info");
  ...
  render() {
    if (
      this.state.sponserImageUploadSuccess &&
      this.state.sponserTimelineImageUploadSuccess
    ) {
      this.successHandler();
    }
    return (
      <div className={classes.EditSponser}>
        ...
        <form ref={this.formRef} className={classes.Form}>
          ...
          <TextField
            ...
            name="link"
            label="Sponser Link"
            value={this.state.initialValues.link.value}
            onChange={(sponser) =>
          ...
          <FormControl variant="outlined" fullWidth>
            ...
            <Select
              ...
              value={this.state.initialValues.size.value}
              onChange={(event) =>
...
const mapDispatchToProps = (dispatch) => {
  return {
    onFetchSponsers: () => dispatch(actions.fetchSponsers()),
    onEditSponser: (sponserData, cb) =>
      dispatch(actions.editSponser(sponserData, cb)),
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(EditSponser)
);
