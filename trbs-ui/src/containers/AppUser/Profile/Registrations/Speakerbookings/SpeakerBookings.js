...
import * as classes from "./SpeakerBookings.module.css";
...
class SpeakerBookings extends Component {
  state = {
    rows: [],
    customGridFilters: {
      status: "success",
  ...
  render() {
    let speakers = this.props.data
      .filter((d) => d.product_type === "connexion")
      .filter((d) => {
      ...
      .map((speaker, index) => {
        return {
          id: index + 1,
          productImage: speaker.product_image,
          productName: speaker.product_name,
          ticketNo: speaker.ticket_no,
          time: speaker.ticket_no ? speaker.time : null,
          venue: speaker.ticket_no ? speaker.venue : null,
          status: speaker.status,
          orderId: speaker.order_id,
        };
      });
    return (
      <div>
